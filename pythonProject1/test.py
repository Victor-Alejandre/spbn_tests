...
SPBNetwork_bcv_G_0 = x.estimate_wbm(operators=operators, score=pb.HoldoutLikelihood(sample, seed=1),
                                start=G_0, m_bselector=pb.SCV(), epsilon=0.01)
SPBNetwork_bcv_G_0.fit_wbm(sample, pb.SCV())
...
print(result_SP_bcv_G_0)
result_SP_bcv_G_1= ['G_1', 'hc_bcv' ]
SPBNetwork_bcv_G_1 = pb.SemiparametricBN(['A', 'B', 'C', 'D', 'E'])
x = pb.GreedyHillClimbing()
SPBNetwork_bcv_G_1 = x.estimate_wbm(operators=operators, score=pb.HoldoutLikelihood(sample, seed=1),
                                start=G_1, m_bselector=pb.SCV(), epsilon=0.01)
SPBNetwork_bcv_G_1.fit_wbm(sample, pb.SCV())
