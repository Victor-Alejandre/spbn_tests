...
def structural_hamming_distance(pdag1, pdag2):
    """
    Calculate the Structural Hamming Distance (SHD) between two PDAGs.

    Parameters:
    pdag1, pdag2: PDAG
        The two PDAGs to be compared.

    Returns:
    int
        The Structural Hamming Distance between the two PDAGs.
    """
    # Ensure both PDAGs have the same nodes
    assert set(pdag1.nodes()) == set(pdag2.nodes()), "PDAGs must have the same nodes"

    shd = 0

    edges1 = set(pdag1.edges())
    edges2 = set(pdag2.edges())
    arcs1 = set(pdag1.arcs())
    arcs2 = set(pdag2.arcs())
    ...
    for arc in arcs1:
        if arc not in arcs2:
            if (arc[1], arc[0]) not in edges2 and (arc[0], arc[1]) not in edges2:
                shd += 1

    for arc in arcs2:
        if arc not in arcs1:
            if (arc[1], arc[0]) not in edges1 and (arc[0], arc[1]) not in edges1:
                shd += 1
...
for k in range(0, 1):
    ...
    SPBNetwork_scv_G_1 = x.estimate_wbm(operators=operators, score=pb.ValidatedLikelihood(sample, seed=10, k = 5),
    ...
    result_SP_scv_G_1.append(structural_hamming_distance(pb.Dag(SPBNetwork_scv_G_1.nodes(), SPBNetwork_scv_G_1.arcs()).to_pdag(), pb.Dag(ground_truth_network.nodes(),ground_truth_network.arcs()).to_pdag()))
    ...
    print(results)
    results.to_csv('results_exp_1_scv_G1_10000_0.csv')